...
try
{
    ...
    $eocdBytes = $response.Content
    $SizeOfCD = [BitConverter]::ToUInt32($eocdBytes, 12)
    $OffsetOfCD = [BitConverter]::ToUInt32($eocdBytes, 16)

    # $SizeOfCD
    # $OffsetOfCD

    $headers = @{
        'Range' = "bytes=$($OffsetOfCD)-$($OffsetOfCD+$SizeOfCD-1)"
    }
    $CentralDirectory = Invoke-WebRequest -Uri $DUrl -Headers $headers
    $cdbytes = $CentralDirectory.Content

    $CDPlusEOCDBytes = [System.Linq.Enumerable]::Concat($cdBytes, $eocdbytes).ToArray()

    $MemoryStream = [System.IO.MemoryStream]::new($CDPlusEOCDBytes)
